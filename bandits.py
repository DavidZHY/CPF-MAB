...
class PFEDUCB(object):
    def __init__(self,
        ...
        self.reward_type = 'Gaussian' # 模拟奖励使用的方法
        ...
        self.clients = [
            client(index = i,
                   ...
                   palpha = self.alpha,
                   fp=fun_fp,
                   local_mean=c_local_mean[i],
                   global_mean=c_global_mean[i],
                   mixed_mean=c_mixed_mean[i]
    ...
    def simulate_single_step_rewards(self):
        ...
        return np.random.normal(self.local_means, 0.1)
...
#        print('regret:',regret[-1], "comm: ", self.comm)
    ...
    def get_clients_data(self):
        ...
        c_mixed_mean = []
        for i in range(self.M):
            c_local_mean.append(self.clients[i].send_all_data()[0])
            c_global_mean.append(self.clients[i].send_all_data()[1])
            c_mixed_mean.append(self.clients[i].send_all_data()[2])
        return c_local_mean, c_global_mean, c_mixed_mean